...
| --- | ---: | ---: | ---: | ---: | ---: |
| 4 generation / 4 prompt threads, batch 256/64, FP16 KV (before installation) | 1.100 s | 11.959 s | 635 MiB | 618 MiB | 0/48 |
| Same settings, independent post-install replay | 1.223 s | 11.854 s | 633 MiB | 633 MiB | 0/48 |
...
The original FP16 setting stayed within the 1.5 GiB service budget and was fastest in the pre-install configuration screen, so the installer retained it. The separate installed replay was 0.123 s slower at the median and 0.106 s faster at p95; this single pair of runs does not establish a runtime change. Its peak/retained RSS was 633/633 MiB versus 635/618 MiB before installation, with zero process swap in both windows. Q8 KV saved roughly 84 MiB at the observed peak but increased median and tail latency in its screen. The equal output hashes are a narrow synthetic regression observation, not proof of model quality. Same-prompt cached requests completed in 0.362, 0.382, and 0.482 s across the pre-install baseline buckets; the backend reported a median 960 cached input tokens in both changed-state replays. Those same-prompt numbers are not everyday edit latency. In the real selected-model synthetic editor smoke, prompt lengths were 132–134 tokens, backend `cache_n` was 131–133, and last edit to completed display was 402, 375, and 386 ms, including the debounce. The smoke is short and favorable to prefix reuse.
