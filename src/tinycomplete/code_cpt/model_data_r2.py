...
def line_source_rejection(record):
    """Diagnostic-only exclusions; do not change either frozen training policy."""
    text, path = record["content"], record["path"]
    if PATHS.search(path) or MARKERS.search(text):
        return "generated_or_vendor_marker"
    if re.search(
        r"\b(?:auto[- ]?generated|machine[- ]generated|generated\s+(?:by|from|using|with))\b"
        r"|@generated\b|<auto-generated>|\bCode generated\b",
        text,
        re.I,
    ) or re.search(r"(?:\.pb\.(?:cc|h|go)|\.g\.cs|\.designer\.cs|_generated\.[^/]+)$", path, re.I):
        return "generated_tool_signature"
    if re.search(r"(?:^|/)html/search/[^/]+\.js$", path, re.I) or (
        re.search(r"\bvar\s+(?:searchData|NAVTREEINDEX\d*|NAVTREE)\s*=", text)
    ):
        return "generated_documentation_index"
    if len(re.findall(r"\b0x[0-9a-fA-F]{2}\b", text)) >= 64 and re.search(
        r"(?:unsigned\s+)?char\s+\w+\s*\[\s*\]\s*=\s*\{", text
    ):
        return "generated_embedded_byte_array"
    return None


def line_candidate(record):
    """One deterministic target per held-out file, before observing any model output."""
    if record["parse_status"] != "pass" or line_source_rejection(record):
        return None
    text = record["content"]
